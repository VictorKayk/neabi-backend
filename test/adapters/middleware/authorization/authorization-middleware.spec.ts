...
const makeSut = (): SutTypes => {
  ...
  const sut = new AuthorizationMiddleware(useCase, ['any_role']);
...
describe('Authorization Middleware', () => {
  it('Should call AuthorizationUseCase with correct values', async () => {
    ...
    await sut.handle({ userId: 'any_userId' });
    expect(useCaseSpy).toHaveBeenCalledWith({ userId: 'any_userId', allowedRoles: ['any_role'] });
  });
  ...
  it('Should return 500 if use case throws', async () => {
    ...
    const response = await sut.handle({ userId: 'any_userId' });
  ...
  it('Should return 401 if call AuthorizationUseCase with incorrect values', async () => {
    ...
    const response = await sut.handle({ userId: 'invalid_userId' });
